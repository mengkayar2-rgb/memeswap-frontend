import { ChainId, Token } from '@memeswap/sdk'
...
// Monad Mainnet Tokens - MMF is the main token (like CAKE)
// Using 'as any' to bypass ChainId type check since Monad (143) is not in SDK enum
...
// Monad tokens mapped to standard names (cake = MMF, wvlx = WMON)
export const monadTokens = {
  wmon: MONAD_TOKENS.WMON,
  wvlx: MONAD_TOKENS.WMON, // alias for compatibility
  mon: new Token(MONAD as any, addresses[143].WVLX, 18, 'MON', 'MON', 'https://monad.xyz/'),
  cake: MONAD_TOKENS.MMF, // MMF is the main token (like CAKE)
  syrup: MONAD_TOKENS.SYRUP,
  // BUSD placeholder - using MMF for price calculations on Monad
  busd: MONAD_TOKENS.MMF,
}
...
const tokens = () => {
  ...
  // If Monad - return monad tokens
  if (parseInt(chainId, 10) === MONAD) {
    return Object.keys(monadTokens).reduce((accum, key) => {
      return { ...accum, [key]: monadTokens[key] }
    }, {} as typeof monadTokens)
  }
