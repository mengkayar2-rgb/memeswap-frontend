...
// Monad chainId
const MONAD = 143
const isMonad = CHAIN_ID === String(MONAD)
const coreFarmPIDs = isMonad ? [] : CHAIN_ID === String(ChainId.MAINNET) ? [2, 3] : [1, 2]

export const usePollCoreFarmData = () => {
  const dispatch = useAppDispatch()

  useFastRefreshEffect(() => {
    // Skip farm polling for Monad (no farms configured yet)
    if (isMonad || coreFarmPIDs.length === 0) return
