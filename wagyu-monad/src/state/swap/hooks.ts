import { Currency, CurrencyAmount, ETHER, Token, Trade } from '@memeswap/sdk'
...
const NATIVE_CURRENCY = parseInt(CHAIN_ID, 10) === 143 ? 'MON' : 'VLX'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER ? NATIVE_CURRENCY : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    // Support both VLX (Velas) and MON (Monad) as native currency
    if (urlParam.toUpperCase() === 'VLX' || urlParam.toUpperCase() === 'MON') return urlParam.toUpperCase()
    if (valid === false) return 'MON' // Default to MON for Monad
  }
  return ''
}
