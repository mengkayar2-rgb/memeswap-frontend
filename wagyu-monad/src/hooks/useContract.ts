...
// Monad WMON address (WETH equivalent)
const MONAD_WETH_ADDRESS = '0xFfDFF1bAb47d8e6D1Da119F5C925fAC91FAfA899'
const MONAD_CHAIN_ID = 143

export function useWBNBContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  
  // Get WETH address - handle Monad chain (143) separately since it's not in SDK
  const getWethAddress = () => {
    if (!chainId) return undefined
    if (chainId === MONAD_CHAIN_ID) return MONAD_WETH_ADDRESS
    return WETH[chainId]?.address
  }
  
  return useContract<Weth>(getWethAddress(), WETH_ABI, withSignerIfPossible)
}
