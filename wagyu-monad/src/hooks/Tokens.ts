...
import { Currency, currencyEquals, ETHER, Token } from '@memeswap/sdk'
...
const mapWithoutUrls = (tokenMap: TokenAddressMap) => {
  // Handle undefined tokenMap for chain (e.g., Monad 143 not in token list)
  if (!tokenMap || !tokenMap[CHAIN_ID]) {
    return {}
  }
  return Object.keys(tokenMap[CHAIN_ID]).reduce<{ [address: string]: Token }>((newMap, address) => {
...
export function useCurrency(currencyId: string | undefined): Currency | Token | null | undefined {
  // Check for native currency - VLX for Velas, MON for Monad
  const isBNB = currencyId?.toUpperCase() === 'VLX' || 
                currencyId?.toUpperCase() === 'MON' || 
