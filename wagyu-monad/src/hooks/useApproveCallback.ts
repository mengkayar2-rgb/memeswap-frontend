...
export function useApproveCallback(
  ...
  const approve = useCallback(async (): Promise<void> => {
    ...
    // For Monad chain, use manual gas settings to avoid Internal JSON-RPC error
    if (isMonadChain) {
      gasLimit = MONAD_GAS_LIMIT
      console.log('[Monad] Using manual gas settings for approve:', {
        gasLimit: gasLimit.toString(),
        gasPrice: MONAD_GAS_PRICE.toString(),
      })
    } else {
    ...
    // Build gas overrides - use manual settings for Monad
    const gasOverrides = isMonadChain
      ? {
          gasLimit: MONAD_GAS_LIMIT,
    ...
    return callWithGasPrice(
      ...
      .catch((error: any) => {
        ...
        // Enhanced error handling for Monad
        if (error?.code !== 4001) {
          const errorMessage = error?.data?.message || error?.message || 'Unknown error'
          
          // Check for common Monad errors
          if (errorMessage.includes('Internal JSON-RPC') || errorMessage.includes('execution reverted')) {
            toastError(
              t('Error'),
              t('Please add Monad network (Chain ID: 143) to your wallet and try again'),
            )
          } else {
