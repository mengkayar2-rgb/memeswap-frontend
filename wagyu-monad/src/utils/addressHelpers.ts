...
export const getAddress = (address: Address): string => {
  ...
  // Prioritize Monad, then current chainId, then fallback to mainnet
  if (address[chainId]) return address[chainId]
  if (parseInt(chainId, 10) === MONAD && address[MONAD]) return address[MONAD]
