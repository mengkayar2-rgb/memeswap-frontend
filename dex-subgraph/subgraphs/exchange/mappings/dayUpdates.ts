...
export function updatePancakeDayData(event: ethereum.Event): PancakeDayData {
  ...
  if (pancake === null) {
    // Return a new empty day data if factory doesn't exist
    let timestamp = event.block.timestamp.toI32();
    let dayID = timestamp / 86400;
    let dayStartTimestamp = dayID * 86400;
    let pancakeDayData = new PancakeDayData(dayID.toString());
...
export function updatePairDayData(event: ethereum.Event): PairDayData {
  ...
  if (pair === null) {
    // Return empty day data if pair doesn't exist
...
export function updatePairHourData(event: ethereum.Event): PairHourData {
  ...
  let pair = Pair.load(event.address.toHex());
  if (pair === null) {
    // Return empty hour data if pair doesn't exist
...
export function updateTokenDayData(token: Token, event: ethereum.Event): TokenDayData {
  ...
  let tokenDerivedBNB = token.derivedBNB !== null ? token.derivedBNB! : ZERO_BD;
  if (tokenDayData === null) {
    tokenDayData = new TokenDayData(tokenDayID);
    tokenDayData.date = dayStartTimestamp;
    tokenDayData.token = token.id;
    if (bundle !== null) {
      tokenDayData.priceUSD = tokenDerivedBNB.times(bundle.bnbPrice);
    } else {
  ...
  if (bundle !== null) {
    tokenDayData.priceUSD = tokenDerivedBNB.times(bundle.bnbPrice);
    tokenDayData.totalLiquidityBNB = token.totalLiquidity.times(tokenDerivedBNB);
