...
export function useSwapBestTrade({ maxHops }: Options = {}) {
  ...
  const v3SwapForChain = chainId === 143 ? false : v3Swap
  const stableSwapForChain = chainId === 143 ? false : stableSwapEnable
  ...
  const { isLoading, trade, refresh, syncing, isStale, error } = useBestAMMTrade({
    ...
    v2Swap,
    v3Swap: v3SwapForChain,
    stableSwap: stableSwapForChain,
    type: 'auto',
    trackPerf: true,
  })
